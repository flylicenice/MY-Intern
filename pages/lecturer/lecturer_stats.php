<div class="dashboard-header-section" style="margin-bottom: 2rem; ">
    ...
    <p class="total-counter-subtitle" style="color: #64748b; font-size: 14px; margin-top: 4px;">
        Total Students: <strong><?php echo (int)$total_students; ?> Students</strong>
    </p>
</div>
...
<div class="filter-buttons-container" style="margin: 1.5rem 0; display: flex; gap: 12px; align-items: center; flex-wrap: wrap; width: 100%;">
    ...
    <button class="filter-btn" data-filter="placed" style="background-color: #fff; color: #334155; border: 1px solid #cbd5e1; padding: 8px 16px; border-radius: 6px; font-weight: 500; cursor: pointer; transition: all 0.2s;">Placed (<?php echo (int)$status_counts['Placed']; ?>)</button>
    <button class="filter-btn" data-filter="still-applying" style="background-color: #fff; color: #334155; border: 1px solid #cbd5e1; padding: 8px 16px; border-radius: 6px; font-weight: 500; cursor: pointer; transition: all 0.2s;">Still Applying (<?php echo (int)$status_counts['Still Applying']; ?>)</button>
    <button class="filter-btn" data-filter="not-applying" style="background-color: #fff; color: #334155; border: 1px solid #cbd5e1; padding: 8px 16px; border-radius: 6px; font-weight: 500; cursor: pointer; transition: all 0.2s;">Not Applying (<?php echo (int)$status_counts['Not Applying']; ?>)</button>
</div>
    ...
    <table class="lecturer-students-table" style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead>
            <tr style="border-bottom: 2px solid #edf2f7; color: #4a5568; font-size: 13px; text-transform: uppercase; background: #f8fafc;">
                ...
                <th style="padding: 14px 12px;">Placed Company / Info</th>
        ...
        <tbody>
            <?php if ($table_result && $table_result->num_rows > 0): ?>
                <?php while ($student = $table_result->fetch_assoc()): 
                    // Dynamic layout normalization maps
                    $status = strtoupper(trim($student['intern_status']));
                    $pendingCount = (int)$student['pending_count'];

                    // Define combined JS row dataset flags
                ...
                ?>
                    <tr class="student-row" data-status="<?php echo $row_status; ?>" style="border-bottom: 1px solid #edf2f7;">
                        <td class="font-bold" style="padding: 16px 12px; font-weight: 700;">
                            <?php echo htmlspecialchars($student['matric_number']); ?>
                        </td>
                        <td class="student-name-cell" style="padding: 16px 12px; font-weight: 500; text-transform: capitalize;">
                        ...
                        <td style="padding: 16px 12px;">
                            ...
                            if ($row_status === 'placed') {
                                echo htmlspecialchars($student['current_placement_company'] ?? 'Assigned Company');
                            } elseif ($pendingCount > 0) {
                                echo $pendingCount . ' Pending Applications';
                            } else {
                        ...
                        <td style="text-align: right; padding: 16px 12px;">
                            <?php if ($row_status === 'not-applying'): ?>
                                <button type="button" class="send-alert-btn" data-id="<?php echo htmlspecialchars($student['matric_number']); ?>" style="background-color: #dc3545; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px; font-weight: 500;">
                                    Send Alert Email
                                </button>
                            <?php else: ?>
                                <button type="button" disabled class="send-alert-btn" data-id="<?php echo htmlspecialchars($student['matric_number']); ?>" style="background-color: grey; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-size: 12px; font-weight: 500;">
                                    Send Alert Email
                                </button>
                            <?php endif; ?>
                        </td>
